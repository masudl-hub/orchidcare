        ...
        <div className="flex gap-2">
          {['ALL', 'USER', 'AGENT', 'SYSTEM'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-full text-[10px] font-mono border transition-all ${filter === f ? 'bg-black text-white border-black' : 'bg-white text-stone-500 border-stone-200 hover:border-black'
                }`}
            >
              {f}
            </button>
            ...
            <div className="space-y-8">
              {filteredLogs.map((activity) => (
                <div key={activity.id} className="relative pl-16 group">
                  {/* Timeline Node */}
                  <div className={`absolute left-4 top-0 w-5 h-5 -translate-x-1/2 rounded-full border flex items-center justify-center z-10 bg-white transition-all group-hover:scale-110 ${getActivityCategory(activity) === 'agent' ? 'border-black' : 'border-stone-400'
                    }`}>
          ...
          <div className="flex flex-col">
            {[
              ...
              <button
                ...
                className={`h-12 flex items-center justify-center md:justify-start md:px-6 hover:bg-stone-100 transition-colors ${activeTab === item.id ? 'bg-black text-white hover:bg-stone-800' : 'text-stone-500'
                  }`}
              >
